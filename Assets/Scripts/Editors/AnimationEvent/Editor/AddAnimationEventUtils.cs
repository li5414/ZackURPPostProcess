...
namespace AnimationEventEditor
{
	class AnimationClipInfoProperties
	{
		...
		public AnimationEvent GetEvent(AnimationClip clip, int index)
		{
			...
			if (events != null && events.isArray)
			{
				if (index < events.arraySize)
				{
					...
					evt.time = events.GetArrayElementAtIndex(index).FindPropertyRelative("time").floatValue * clip.length;	// meta文件中的time值存的是normalizedTime，是百分比
				}
				else
				{
		...
		public void SetEvent(AnimationClip clip, int index, AnimationEvent animationEvent)
		{
			SerializedProperty events = Get("events");
			...
			if (events != null && events.isArray)
			{
				if (index < events.arraySize)
				{
					...
					events.GetArrayElementAtIndex(index).FindPropertyRelative("time").floatValue = animationEvent.time / clip.length;	// meta文件中的time值存的是normalizedTime，是百分比
				}
				else
				{
					Debug.LogWarning("Invalid Event Index");
		...
		public void SetEvents(AnimationClip clip, AnimationEvent[] newEvents)
		{
			SerializedProperty events = Get("events");
			...
			if (events != null && events.isArray)
			{
				...
				foreach (AnimationEvent evt in newEvents)
				{
					...
					SetEvent(clip, events.arraySize - 1, evt);
				}
			}
		}

		public AnimationEvent[] GetEvents(AnimationClip clip)
		{
	...
	public class AddAnimationEventUtils 
	{
		/// <summary>
		/// 修改动画的帧事件 (保存进meta)
		/// </summary>
		...
		static void DoAddEventImportedClip(AnimationClip sourceAnimClip, AnimationClip targetAnimClip)
		{
			...
			for (int i = 0; i < clipAnimations.arraySize; i++)
			{
				...
				if (clipInfoProperties.name == targetAnimClip.name)
				{
					...
					clipInfoProperties.SetEvents(targetAnimClip, sourceAnimEvents);
					serializedObject.ApplyModifiedProperties();
