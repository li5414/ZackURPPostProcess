...
namespace Zack.Editor
{
    ...
    /// 编辑器参数
    /// </summary>
    public class BayerDither : EditorWindow
    {
        [MenuItem("Editor/Texture/贝尔抖动纹理", isValidateFunction:false)]
        static void OpenEditor()
        {
            EditorWindow.GetWindow<BayerDither>()._ExportPath = Application.dataPath + "/BayerDither.png";
            EditorWindow.GetWindow<BayerDither>().Show();
        }
        ...
        void OnGUI()
        {
            ...
            {
                float[] bayerMatrix = new float[_Size*_Size];
                // 生成抖动数据
                GenerateBayerDitherMatrix(ref bayerMatrix, _Size);
                // 生成Texture2D
                var tex2D = new Texture2D(_Size, _Size, TextureFormat.R8, false);
                tex2D.filterMode = FilterMode.Point;
                for (int x = 0; x < _Size; ++x)
                {
                    for (int y = 0; y < _Size; ++y)
                    {
                        float value = bayerMatrix[_Size * y + x];
                        tex2D.SetPixel(x, y, new Color(value, 0, 0, 0));
                    }
                }
                // 保存
